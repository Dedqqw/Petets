...
pygame.display.set_caption("PetetsV8")
...
menu_bg = pygame.image.load("fotos/anthony.png")
...
broken_heart_img = pygame.transform.scale(pygame.image.load("fotos/broken-heart.png"), (32,32))
...
block_skins = {
    "Sam": {"img": pygame.transform.scale(pygame.image.load("fotos/penmakers.png").convert_alpha(), (100,100)), "price": 0},
    "heil Nick": {"img": pygame.transform.scale(pygame.image.load("fotos/nick.png").convert_alpha(), (100,100)), "price": 50},
}
owned_blocks= save["owned_blocks"]
current_blocks = save["current_blocks"]
...
skin_btn = Button("SKINS PEDDLE", 1, 360, 200, 45)
bg_btn = Button("BG SKINS", 400, 360, 150, 45)
blocks_btn = Button("BLOCK_SKINS", 200, 360, 200, 45)
...
def skin_button(x, y, w, h, text, selected=False):
    color = (150, 255, 150) if selected else GRAY
...
while run:
    ...
    for e in pygame.event.get():
        ...
        if state == MENU:
            ...
            if leader_btn.clicked(e):
                state = LEADERBOARD
            if skin_btn.clicked(e):
                state = SKINS_PEDDLE
            if bg_btn.clicked(e):
                state = SKINS_BG
            if blocks_btn.clicked(e):
                state = SKINS_BLOCKS

        elif state == SHOP:
            if skin_btn.clicked(e) or e.type == pygame.KEYDOWN and e.key == pygame.K_ESCAPE:
                state = SKINS_PEDDLE
                save_game()
            if back_btn.clicked(e) or e.type == pygame.KEYDOWN and e.key == pygame.K_ESCAPE:
                state = MENU
        ...
        if state == SKINS_BLOCKS:
            if back_btn.clicked(e) or e.type == pygame.KEYDOWN and e.key == pygame.K_ESCAPE:
                state = SHOP
                save_game()
            if e.type == pygame.MOUSEBUTTONDOWN:
                y = 200
                for name, blocks in block_skins.items():
                    rect = pygame.Rect(120, y, 300, 30)
                    if rect.collidepoint(e.pos):
                        if name in owned_blocks:
                            current_blocks = name
                        elif money >= blocks["price"]:
                            money -= blocks["price"]
    ...
    if state == MENU:
        #screen.blit(title_font.render("     Petets", True, BLACK), (W//2-110, 120))
        title_txt = title_font.render("PETETS", True, BLACK)
        screen.blit(title_txt, title_txt.get_rect(center=(W // 2, 170)))
        play_btn.draw()
        shop_btn.draw()
        leader_btn.draw()
        ...
        screen.blit(title_txt, title_txt.get_rect(center=(W // 2, 200)))
        blocks_btn.draw()
        skin_btn.draw()
        back_btn.draw()
        bg_btn.draw()
        ...
        screen.blit(title_font.render("SKINS", True, BLACK), (W//2-60, 120))
        ...
        screen.blit(title_font.render("BLOCK SKINS", True, BLACK), (W//2-130, 120))
        y = 200
        for name, blocks in block_skins.items():
            if name == current_blocks:
                status = "SELECTED"
            elif name in owned_blocks:
                status = "OWNED"
            else:
                status = f"{blocks['price']} coins"
